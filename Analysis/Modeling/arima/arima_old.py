...
def predict_on_match_id(model, x_mat, longform_df, date, match_id,
                        opponent_match_id, model_no, filename_out):
    ...
    start_idx = int(min(match_data.index))
    end_idx = int(max(match_data.index))
    start_date = match_data.iloc[0].date_time
    end_date = match_data.iloc[-1].date_time
    print(start_date)
    print(end_date)
    print(match_data)
    print(x_mat.iloc[start_idx:end_idx + 1])
    ...
    preds = [(model_no, pred) for pred in model.predict(start=start_date,
                                                        end=end_date,
                                                        exog=x_mat)]
